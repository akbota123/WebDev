...
import {ITaskList, ITaskShort, ITaskLong, ITaskNew, IAuthResponse} from '../models/models';
...
export class ProviderService extends MainService {
  ...
  createTask(taskListId: number, name: string, due_on: string, status: string): Promise<ITaskNew> {
    return this.post(`http://localhost:8000/task_lists/${taskListId}/tasks/`, {
      name: name,
      due_on: due_on,
      status: status
    });
  ...
  updateTask(task: ITaskNew): Promise<ITaskNew> {
    return this.put(`http://localhost:8000/tasks/${task.id}/`, {
      name: task.name,
      due_on: task.due_on,
      status: task.status
    });
  ...
  deleteTask(id: number): Promise<any> {
    return this.delet(`http://localhost:8000/tasks/${id}/`, {});
  }
